...
# ============================================================================
# CloudX Meta Adapter - Static XCFramework Release Script
# ============================================================================
#
# OVERVIEW:
#   Builds CloudXMetaAdapter as a STATIC xcframework and prepares for binary distribution.
#   This is used by CI/CD but can also be run locally for testing.
#
# USAGE:
#   cd cloudx-ios-private/adapter-meta
#   ./release-meta-xcframework.sh 1.2.0
#
# WHAT IT DOES:
...
#
# OUTPUT ARTIFACTS:
#   - CloudXMetaAdapter.xcframework/          (unzipped framework)
#   - CloudXMetaAdapter.xcframework.zip       (distributable binary)
...
#   2. Referenced in CloudXMetaAdapter.podspec with version and download URL
...
FRAMEWORK_NAME="CloudXMetaAdapter"
SCHEME_NAME="CloudXMetaAdapter"
WORKSPACE_FILE="CloudXMetaAdapter.xcworkspace"
...
create_xcframework() {
    ...
    # Extract static libraries from frameworks and rename to .a (AppLovin-style)
    ...
    # Create xcframework with raw static libraries (like AppLovin does)
...
CloudX Meta Adapter Release Metadata
====================================
...
--------------
Tag: v${version}-meta
Release Title: CloudX Meta Adapter v${version}

Download URL (replace with actual after upload):
https://github.com/cloudx-io/cloudx-ios/releases/download/v${version}-meta/${FRAMEWORK_NAME}.xcframework.zip

CocoaPods Podspec
-----------------
Update CloudXMetaAdapter.podspec:

  s.version = '${version}'
  s.source = {
    :http => 'https://github.com/cloudx-io/cloudx-ios/releases/download/v${version}-meta/${FRAMEWORK_NAME}.xcframework.zip',
    :sha256 => '<compute with: shasum -a 256 ${FRAMEWORK_NAME}.xcframework.zip>'
  }
  s.vendored_frameworks = 'adapter-meta/${FRAMEWORK_NAME}.xcframework'
...
Update Package.swift:
  ...
  .binaryTarget(
    name: "${FRAMEWORK_NAME}",
    url: "https://github.com/cloudx-io/cloudx-ios/releases/download/v${version}-meta/${FRAMEWORK_NAME}.xcframework.zip",
    checksum: "${checksum}"
  )

Next Steps
----------
1. Upload ${FRAMEWORK_NAME}.xcframework.zip to GitHub Release v${version}-meta
2. Update podspec in cloudx-ios/adapter-meta/CloudXMetaAdapter.podspec
3. Update Package.swift in cloudx-ios/adapter-meta/
4. Push podspec to CocoaPods trunk: pod trunk push CloudXMetaAdapter.podspec
...
main() {
    print_header "CloudX Meta Adapter - Static XCFramework Build"
    ...
    print_info "Next: Copy ${FRAMEWORK_NAME}.xcframework to cloudx-ios/adapter-meta/"
}
