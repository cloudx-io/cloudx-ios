...
//  CloudXRenderer
//
//  Renderer native ad implementation
//
...
@implementation CLXRendererNative
...
- (instancetype)initWithAdm:(NSString *)adm
    ...
    CLXLogger *logger = [[CLXLogger alloc] initWithCategory:@"CLXRendererNative"];
    [logger debug:[NSString stringWithFormat:@"[INIT] CLXRendererNative initialization - Markup: %lu chars, Type: %ld", (unsigned long)adm.length, (long)type]];
    ...
    if (self) {
        ...
        [logger info:@"CLXRendererNative initialization completed"];
    } else {
...
- (void)load {
    CLXLogger *logger = [[CLXLogger alloc] initWithCategory:@"CLXRendererNative"];
    [logger debug:@"[LOAD] CLXRendererNative load() method called"];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([self.delegate respondsToSelector:@selector(didLoadWithNative:)]) {
                [logger info:@"[CLXRendererNative] load successful, notifying delegate."];
...
- (void)showFromViewController:(UIViewController *)viewController {
    CLXLogger *logger = [[CLXLogger alloc] initWithCategory:@"CLXRendererNative"];
    [logger debug:@"[CLXRendererNative] showFromViewController called"];
