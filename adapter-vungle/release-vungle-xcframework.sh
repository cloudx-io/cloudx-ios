...
# ============================================================================
# CloudX Vungle Adapter - Static XCFramework Release Script
# ============================================================================
#
# OVERVIEW:
#   Builds CloudXVungleAdapter as a STATIC xcframework and prepares for binary distribution.
#   This is used by CI/CD but can also be run locally for testing.
#
# USAGE:
#   cd cloudx-ios-private/adapter-vungle
#   ./release-vungle-xcframework.sh 1.2.0
#
# WHAT IT DOES:
...
#
# OUTPUT ARTIFACTS:
#   - CloudXVungleAdapter.xcframework/          (unzipped framework)
#   - CloudXVungleAdapter.xcframework.zip       (distributable binary)
...
#   2. Referenced in CloudXVungleAdapter.podspec with version and download URL
...
WORKSPACE_FILE="CloudXVungleAdapter.xcworkspace"
...
CloudX Vungle Adapter Release Metadata
======================================
...
GitHub Release
--------------
Tag: v${version}-vungle
Release Title: CloudX Vungle Adapter v${version}

Download URL (replace with actual after upload):
https://github.com/cloudx-io/cloudx-ios/releases/download/v${version}-vungle/${FRAMEWORK_NAME}.xcframework.zip

CocoaPods Podspec
-----------------
Update CloudXVungleAdapter.podspec:

  s.version = '${version}'
  s.source = {
    :http => 'https://github.com/cloudx-io/cloudx-ios/releases/download/v${version}-vungle/${FRAMEWORK_NAME}.xcframework.zip',
    :sha256 => '<compute with: shasum -a 256 ${FRAMEWORK_NAME}.xcframework.zip>'
  }
  s.vendored_frameworks = 'adapter-vungle/${FRAMEWORK_NAME}.xcframework'
...
Update Package.swift:
  ...
  .binaryTarget(
    name: "${FRAMEWORK_NAME}",
    url: "https://github.com/cloudx-io/cloudx-ios/releases/download/v${version}-vungle/${FRAMEWORK_NAME}.xcframework.zip",
    checksum: "${checksum}"
  )

Next Steps
----------
1. Upload ${FRAMEWORK_NAME}.xcframework.zip to GitHub Release v${version}-vungle
2. Update podspec in cloudx-ios/adapter-vungle/CloudXVungleAdapter.podspec
3. Update Package.swift in cloudx-ios/adapter-vungle/
4. Push podspec to CocoaPods trunk: pod trunk push CloudXVungleAdapter.podspec
...
main() {
    print_header "CloudX Vungle Adapter - Static XCFramework Build"
    ...
    print_info "Next: Copy ${FRAMEWORK_NAME}.xcframework to cloudx-ios/adapter-vungle/"
}
