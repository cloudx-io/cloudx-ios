...
+ (CLXLogger *)logger {
    ...
    dispatch_once(&onceToken, ^{
        logger = [[CLXLogger alloc] initWithCategory:@"VungleNativeFactory"];
    });
    return logger;
}
...
- (nullable id<CLXAdapterNative>)createWithAdId:(NSString *)adId
    ...
    if (!delegate) {
        [logger error:@"Cannot create native adapter - delegate is nil"];
        return nil;
    }
    ...
    if (!adapter) {
        [logger error:@"Failed to create Vungle native adapter" ];
        return nil;
    }
    
    [logger debug:@"Successfully created Vungle native adapter" ];
    return adapter;
}
