...
@interface CLXTestVastNetworkNative ()
...
@implementation CLXTestVastNetworkNative
...
- (instancetype)initWithAdm:(NSString *)adm
    ...
    self = [super init];
    if (self) {
        CLXLogger *logger = [[CLXLogger alloc] initWithCategory:@"CloudXTestVastNetworkNative"];
        [logger debug:[NSString stringWithFormat:@"[CloudXTestVastNetworkNative] initWithAdm called. adm: %@, type: %ld", adm, (long)type]];
        ...
        if (self.nativeAdData) {
            [logger info:@"[CloudXTestVastNetworkNative] Native ad data parsed successfully."];
        } else {
            [logger error:@"[CloudXTestVastNetworkNative] Failed to parse native ad data from adm."];
        }
    }
    return self;
}
...
- (void)load {
    CLXLogger *logger = [[CLXLogger alloc] initWithCategory:@"CloudXTestVastNetworkNative"];
    [logger debug:@"[CloudXTestVastNetworkNative] load called."];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!self.nativeAdData) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [logger error:@"[CloudXTestVastNetworkNative] load failed: nativeAdData is nil."];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([self.delegate respondsToSelector:@selector(didLoadWithNative:)]) {
                [logger info:@"[CloudXTestVastNetworkNative] load successful, notifying delegate."];
...
- (void)showFromViewController:(UIViewController *)viewController {
    CLXLogger *logger = [[CLXLogger alloc] initWithCategory:@"CloudXTestVastNetworkNative"];
    [logger debug:@"[CloudXTestVastNetworkNative] showFromViewController called."];
