...
@implementation CLXUnityAdsErrorHandler

+ (CLXError *)toCloudXError:(id<UnityAdsError>)error {
    if (!error) {
        return [CLXError errorWithCode:CLXErrorCodeAdapterInternalError
                           description:@"Unity Ads unknown error"];
    }

    // Unity Ads 4.17.0 uses protobuf-based error codes from PublicErrorCode enum.
    ...
    NSString *description = [NSString stringWithFormat:@"[Unity Ads] error %ld: %@",
