...
public final class CloudXMediumNativeView: UIView, CloudXBaseNativeView {
    ...
    let descriptionLabel: UILabel = {
        let label = UILabel()
        label.translatesAutoresizingMaskIntoConstraints = false
        label.text = "Description very long text"
        label.textColor = .black
    ...
    let _mainImageView: UIImageView = {
        let image = UIImageView()
//        image.translatesAutoresizingMaskIntoConstraints = false
        image.contentMode = .scaleAspectFit
        return image
    ...
    /// Returns container for media view such video player.
    public var mediaViewContainer: UIView? = {
    ...
    let adMark: UILabel = {
        ...
        adMark.text = "ad"
        adMark.font = .systemFont(ofSize: 13)
    ...
    public var mainImageView: UIView? { _mainImageView }
    ...
    public var customMediaView: UIView? {
        didSet {
            guard let mediaView = customMediaView,
            let mediaContainer = mediaViewContainer else { return }
            _mainImageView.removeFromSuperview()
            mediaContainer.addSubview(mediaView)
            
            customMediaView?.translatesAutoresizingMaskIntoConstraints = true
            customMediaView?.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        }
    }
    ...
    func setupView(_ hasCloseButton: Bool) {
        guard let mediaViewContainer else { return }
        ...
        addSubview(mediaViewContainer)
        
        mediaViewContainer.addSubview(_mainImageView)
        _mainImageView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        ...
        NSLayoutConstraint.activate([
            ...
            mediaViewContainer.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 8),
            mediaViewContainer.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -8),
            mediaViewContainer.topAnchor.constraint(equalTo: adMark.bottomAnchor, constant: 8),
            mediaViewContainer.bottomAnchor.constraint(equalTo: appIconView.topAnchor, constant: -8),
            ...
            callToAction.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -8),
...
@available(iOS 17, *)
#Preview {
    let size = CloudXNativeTemplate.medium.size
    let view = CloudXMediumNativeView(size: size)
    view.appIcon = UIImage(systemName: "pencil")
    view.mainImage = UIImage(systemName: "circle")
