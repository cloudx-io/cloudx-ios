...
cd core
sed -i '' "s/s\.version.*=.*/s.version          = '$VERSION_NO_SUFFIX'/" CloudXCore-remote.podspec

print_step "🧪 Validate podspec"
pod spec lint CloudXCore-remote.podspec --allow-warnings --skip-import-validation --skip-tests --no-clean
...
for i in {1..5}; do
    if pod trunk push CloudXCore-remote.podspec --allow-warnings --skip-import-validation --skip-tests; then
